...
from custom_list import CostumList
...
class TestCustomList(unittest.TestCase):
    ...
    def test_add_custom_eq_len(self):
        ...
        a = CostumList([i for i in range(1, 10)])
        b = CostumList([j for j in range(9, 0, -1)])
        res = CostumList([10 for _ in range(1, 10)])
        c = a + b
        self.assertListEqual(c, res)
        self.assertIsInstance(c, CostumList)

    def test_add_custom_ne_len(self):
        """Тест сложения двух объектов CustomList разной длины 
        """

        a = CostumList([i for i in range(1, 5)])
        b = CostumList([j for j in range(9, 0, -1)])
        res = CostumList([10, 10, 10, 10, 5, 4, 3, 2, 1])
        c = a + b
        self.assertListEqual(c, res)
        self.assertIsInstance(c, CostumList)
    ...
    def test_lt_custom(self):
        ...
        a = CostumList([i for i in range(1, 10)])  # sum = 45 
        b = CostumList([j for j in range(1, 5)])  # sum = 15
        self.assertLess(b, a)
    ...
    def test_le_custom_equel(self):
        ...
        a = CostumList([i for i in range(1, 10)])  # sum = 45 
        b = CostumList([j for j in range(1, 10)])  # sum = 45
    ...
    def test_eq_custom(self):
        ...
        a = CostumList([i for i in range(1, 11)])  # sum = 45 
        b = CostumList([5 for j in range(11)])  # sum = 45
